const contacts = () => {
  ...
  const firstNumberBlock = block.querySelector(
    ".header-contacts__phone-number-wrap"
  );

  const secondNumberBlock = block.querySelector(
    ".header-contacts__phone-number-accord"
  );
  const arrowSelector = ".header-contacts__arrow";

  const hiddenPhoneNumber = secondNumberBlock.querySelector(
  ...
  block.addEventListener("click", (e) => {
    ...
    if (target.closest(arrowSelector)) {
      const { height: firstNumberBlockHeight } =
        getComputedStyle(firstNumberBlock);
      const active = secondNumberBlock.classList.toggle("active");
      const { top, opacity, transform } = handlerSecondNumber(
        active,
        firstNumberBlockHeight
      );
